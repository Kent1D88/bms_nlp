...
dict_pl_sau = dict(
    gb_lcr = [r'\b\d{1,5} ?(?= ?elements?)',
              r'(?<=\belement)s?[\s|:]{1,4}\d{1,5}\W',
              r'(?<=\bleucocyte)s?[\s|:]{1,4}\d{1,5}\W',
              r'(?<=\bleu)[\s|:]{1,4}\d{1,5}\W',
              r'(?<=\bleuco)s?[\s|:]{1,4}\d{1,5}\W',
              r'\b\d+(?=\sel\w*)',
              r'(?<=\bgb)\W?\s?\d+\W',
              r'\b\d+(?=\s?leuc?\w*)',
              r'\b\d+(?=\s?gb\s)',
